...
public class PowerPointDbAdapter {
	 ...
	 */
	private static final String DATABASE_CREATE = "create table powerPoints (_id integer primary key autoincrement, "
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		/**
		 * Adds a row to the database with the current time, and the 3 values of
		 * hour, minute and second. is only needed during initialisazion
		 * 
		 * @return rowId or -1 if failed
		 */
		private long addRow(SQLiteDatabase mDb, int hours, int minutes,
				int seconds) {
	...
	public PowerPointRow fetchPowerPointRow(long rowId) throws SQLException {
		...
		mDb.query(true, DATABASE_TABLE, new String[] { KEY_ROWID, KEY_ENDTIME,
				KEY_HOURS, KEY_MINUTES, KEY_SECONDS }, KEY_ROWID + "=" + rowId,
		...
		PowerPointRow row = new PowerPointRow(endTime, mCursor.getInt(2),
	 ...
	 */
	public boolean updatePowerPointRow(long rowId, int delaySeconds, int hours,
